  ...
  try {
    ...
    if (n >= 2 && !_DEBUG) {
      process.stderr.write(
        `[pe-cache-evict] ${peUrl.split("/").pop()} stalled ${n}x — evicting cache\n`,
      );
    }
    if (n >= 2) {
    ...
    if (buf.length > 0) {
      ...
      try {
        ...
        if (stat.size !== buf.length) {
          process.stderr.write(
            `[cache-write-short] ${url} wrote ${stat.size}/${buf.length}b — rewrite\n`,
          );
          fs.writeFileSync(p, buf);
        }
      } catch (err) {
  ...
  return {
    async beforeAsyncRequest({ request, window: w }) {
      ...
      if (/\balicdn\.com/i.test(url)) {
        ...
        if (body && /\.js(\?|$)/i.test(url)) {
          try {
            ...
            process.stderr.write(
              `[cache-bad-js] ${url} len=${body.length} ${parseErr.message} — refetch fresh\n`,
            );
      ...
      try {
        ...
        if (
          _DEBUG &&
          /captcha-open|verify\.|device\.saf|cloudauth-device|upload\./i.test(url) &&
          buf.length &&
          buf.length < 4096
        ) {
          try {
            process.stderr.write(
              `[xhr-body] ${request.method} ${bs.hostname}${bs.pathname}-> ${res.status} ${buf.toString("utf8").slice(0, 1200)}\n`,
            );
          } catch (_) {}
        }
        const headers = {};
      ...
      if (/\balicdn\.com/i.test(url)) {
        ...
        if (body && /\.js(\?|$)/i.test(url)) {
          try {
            ...
            process.stderr.write(
              `[cache-bad-js:sync] ${url} len=${body.length} ${parseErr.message} — refetch fresh\n`,
            );
            _memCdnCache.delete(url);
            try {
              fs.unlinkSync(diskPathFor(url));
          ...
          (async () => {
            try {
              ...
              if (fresh.length > 0 && fresh.length !== src.length) {
                process.stderr.write(
                  `[EVAL-CACHE-MISMATCH] deleting ${diskPathFor(filename)} (stale/truncated cache)\n`,
                );
                try {
...
function applyPolyfills(w) {
  ...
  try {
    Object.defineProperty(w, "alert", { value: w.alert, configurable: true, writable: true });
  } catch (_) {}
  try {
    Object.defineProperty(w, "prompt", { value: w.prompt, configurable: true, writable: true });
  } catch (_) {}
  try {
    Object.defineProperty(w, "confirm", { value: w.confirm, configurable: true, writable: true });
  } catch (_) {}
  try {
    Object.defineProperty(w, "open", { value: w.open, configurable: true, writable: true });
  } catch (_) {}
  try {
    Object.defineProperty(w, "close", { value: w.close, configurable: true, writable: true });
  } catch (_) {}
  ...
  w.requestAnimationFrame =
    w.requestAnimationFrame || ((cb) => w.setTimeout(() => cb(Date.now()), 16));
  ...
  if (!nav.connection) {
    ...
    try {
      Object.defineProperty(nav, "connection", {
        value: makeNS(NetInfo.prototype),
        configurable: true,
      });
    } catch (_) {}
  }
  ...
  try {
    if (!nav.permissions)
      Object.defineProperty(nav, "permissions", {
        value: makeNS(w.Permissions.prototype),
        configurable: true,
      });
  } catch (_) {}
  try {
  ...
  try {
    if (!nav.storage)
      Object.defineProperty(nav, "storage", {
        value: makeNS({
          estimate: () => Promise.resolve({ quota: 1e8, usage: 0 }),
...
async function createDom(region, prefix) {
  ...
  if (!process.__capUnhandledRejectionHooked) {
    ...
    process.on("unhandledRejection", (reason) => {
      ...
      try {
        ...
        process.stderr.write(
          `[host-unhandledRejection] ${typeof reason} ${JSON.stringify(reason).slice(0, 200)} ${r}\n`,
        );
      } catch (_) {}
    });
  ...
  const guestConsole = _DEBUG
    ? {
        log: (...a) =>
          process.stderr.write(
            `[guest-log] ${a.map((x) => (typeof x === "object" ? safeJson(x) : String(x))).join(" ")}\n`,
          ),
        warn: (...a) =>
          process.stderr.write(
            `[guest-warn] ${a.map((x) => (typeof x === "object" ? safeJson(x) : String(x))).join(" ")}\n`,
          ),
        error: (...a) =>
          process.stderr.write(
            `[guest-err] ${a.map((x) => (typeof x === "object" ? safeJson(x) : String(x))).join(" ")}\n`,
          ),
        info: (...a) =>
          process.stderr.write(
            `[guest-info] ${a.map((x) => (typeof x === "object" ? safeJson(x) : String(x))).join(" ")}\n`,
          ),
        debug: (...a) =>
          process.stderr.write(
            `[guest-debug] ${a.map((x) => (typeof x === "object" ? safeJson(x) : String(x))).join(" ")}\n`,
          ),
        trace: (...a) =>
          process.stderr.write(
            `[guest-trace] ${a.map((x) => (typeof x === "object" ? safeJson(x) : String(x))).join(" ")}\n`,
          ),
      }
        ...
        (async () => {
          try {
            ...
            if (fresh.length > 0 && fresh.length !== s.length) {
              process.stderr.write(
                `[${kind}-MISMATCH] deleting ${diskPathFor(url)} (stale/truncated cache)\n`,
              );
              try {
                fs.unlinkSync(diskPathFor(url));
              } catch (_) {}
...
export function makeDualConsole(
  isGuest: () => boolean = isGuestConsoleCall,
...
function destroyDom(win) {
  ...
  try {
    removeGlobalWindowAlias(globalThis, win);
  } catch (_) {}
  try {
...
function extractVerifyParam(param) {
  ...
  if (str.length < 200) {
    throw new Error(
      "verify param too short (" +
        str.length +
        " chars) — degraded result, refusing: " +
        str.slice(0, 80),
    );
  }
  try {
...
function takeReusableWindow() {
  ...
  if (p.solves >= REUSE_MAX_SOLVES) {
    discardReusableWindow();
    return null;
  }
  if (Date.now() - p.lastUsedAt > REUSE_MAX_IDLE_MS) {
    discardReusableWindow();
    return null;
  }
    ...
    const param = await new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        const reqs = _requestLog
          ...
          .map(
            (r) =>
              `${r.at - solveStart}ms ${r.method} ${String(r.url)
                .replace(/^https?:\/\//, "")
                .slice(0, 60)}`,
          )
          .slice(-12);
        reject(
          new Error(
            `captcha solve timeout pe=${peUrl.split("/").pop() || peUrl} reqs=${JSON.stringify(reqs)}`,
          ),
        );
      ...
      const stallTimer = setInterval(() => {
        ...
        if (last && Date.now() - last.at > stallMs) {
          ...
          reject(
            new Error(
              `captcha solve stall pe=${peUrl.split("/").pop() || peUrl} lastXhr=${last.at - solveStart}ms reqs=${JSON.stringify(reqs)}`,
            ),
          );
        }
      }, 500);
